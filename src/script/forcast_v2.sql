...
DELIMITER ;;
CREATE PROCEDURE `forecast_v2`(IN in_forecast_year INT)
BEGIN
...
insert into forecast_tmp(generic_id, forecast_year, stock, min_qty)
select mg.generic_id, in_forecast_year, IFNULL(stock_qty, 0), IFNULL(mg.min_qty, 0)
from mm_generics mg
left join(
...
(
select s.generic_id, sum(s.out_qty) sumy3, count(distinct(month(stock_date))) monthy3
...
update forecast_tmp t,
(
select s.generic_id, sum(s.out_qty) sumy2, count(distinct(month(stock_date))) monthy2
          from wm_stock_card s
...
update forecast_tmp t,
(
select s.generic_id, sum(s.out_qty) sumy1, count(distinct(month(stock_date))) monthy1
          from wm_stock_card s
